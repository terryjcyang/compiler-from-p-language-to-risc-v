...
#include <string>
#include <utility>
#include "sema/SymbolTable.hpp" // MAX_SYMBOL_NAME_LEN

Error::Error(Location p_location) : m_location{p_location} {}

Location Error::getLocation() const {
    return m_location;
}

SymbolRedeclarationError::SymbolRedeclarationError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}

std::string SymbolRedeclarationError::getMessage() const {
    return "symbol '" + m_symbol_name + "' is redeclared";
}

NonPositiveArrayDimensionError::NonPositiveArrayDimensionError(Location p_location,
                                                               std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}

std::string NonPositiveArrayDimensionError::getMessage() const {
    return "'" + m_symbol_name + "' declared as an array with an index that is not greater than 0";
}

UndeclaredSymbolError::UndeclaredSymbolError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}
...
NonVariableSymbolError::NonVariableSymbolError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}
...
NonFunctionSymbolError::NonFunctionSymbolError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}
...
OverArraySubscriptError::OverArraySubscriptError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}
...
InvalidBinaryOperandError::InvalidBinaryOperandError(Location p_location, OperatorType p_op,
                                                     const Type p_lhs, const Type p_rhs)
    : Error{p_location}, m_op{p_op}, m_lhs{p_lhs}, m_rhs{p_rhs} {}

std::string InvalidBinaryOperandError::getMessage() const {
    auto op_name = std::string{OperatorTypeStrings[static_cast<std::size_t>(m_op)]};
    return "invalid operands to binary operator '" + op_name + "' ('" +
           m_lhs.typeToString().c_str() + "' and '" + m_rhs.typeToString().c_str() + "')";
}

InvalidUnaryOperandError::InvalidUnaryOperandError(Location p_location, OperatorType p_op,
                                                   const Type p_operand)
...
std::string InvalidUnaryOperandError::getMessage() const {
    ...
    return "invalid operand to unary operator '" + op_name + "' ('" +
           m_operand.typeToString().c_str() + "')";
}

ArgumentNumberMismatchError::ArgumentNumberMismatchError(Location p_location,
...
IncompatibleArgumentTypeError::IncompatibleArgumentTypeError(Location p_location,
                                                             const Type p_expected,
                                                             const Type p_actual)
    : Error{p_location}, m_expected{std::move(p_expected)}, m_actual{std::move(p_actual)} {}

std::string IncompatibleArgumentTypeError::getMessage() const {
    return "incompatible type passing '" + std::string{m_actual.typeToString().c_str()} +
           "' to parameter of type '" + std::string{m_expected.typeToString().c_str()} + "'";
}
...
AssignToConstantError::AssignToConstantError(Location p_location, std::string p_symbol_name)
    : Error{p_location}, m_symbol_name{std::move(p_symbol_name.substr(0, MAX_SYMBOL_NAME_LEN))} {}

std::string AssignToConstantError::getMessage() const {
    return "cannot assign to variable '" + m_symbol_name + "' which is a constant";
}
...
IncompatibleAssignmentError::IncompatibleAssignmentError(Location p_location, const Type p_lval,
                                                         const Type p_rval)
    : Error{p_location}, m_lval{p_lval}, m_rval{p_rval} {}

std::string IncompatibleAssignmentError::getMessage() const {
    return "assigning to '" + std::string{m_lval.typeToString().c_str()} +
           "' from incompatible type '" + std::string{m_rval.typeToString().c_str()} + "'";
}
...
IncompatibleReturnTypeError::IncompatibleReturnTypeError(Location p_location, const Type p_expected,
                                                         const Type p_actual)
    : Error{p_location}, m_expected{p_expected}, m_actual{p_actual} {}

std::string IncompatibleReturnTypeError::getMessage() const {
    return "return '" + std::string{m_actual.typeToString().c_str()} +
           "' from a function with return type '" + std::string{m_expected.typeToString().c_str()} +
           "'";
}
